<template>
	<view class="qiun-columns">
		...
		<!-- 当图例大于2 -->
		<view class="legend_left" v-if="shop.legend.length>2?true:false">
			<view v-for="(v, i) in shop.legend" :key=i class="layout-box">
				<view class="layout">
		...
		<!-- 当图例小于3 -->
		<view class="legend_center f_r_c" v-if="shop.legend.length<3?true:false">
			<view v-for="(v, i) in shop.legend" :key=i class="layout-box">
				<view class="layout">
					<view class="iconPoint" :style='{background:v.color}'></view>
					{{v.name}}
