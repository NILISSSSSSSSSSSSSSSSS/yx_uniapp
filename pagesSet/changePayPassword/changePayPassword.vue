...
<template>
    <view class="forget_view">
        <view class="forget_cont page">
            <view v-if="payPasswordStatus == 1" class="f_r_s forget_item">
...
<script>
    ...
    import {UpdatePayPwd} from "../../net/SetNet";
    ...
    export default {
        data() {
            return {
                ...
                payPasswordStatus: GetLoginModel.getPayPwd() || 0,
            }
        },

        onLoad() {
            uni.setNavigationBarTitle({
                title: GetLoginModel.getPayPwd() == 1 ? '修改支付密码' : '设置支付密码'
            });
        },
        ...
        methods: {
            ...
            // 确定
            confirmBtn() {
                if(this.payPasswordStatus == 1 && !this.requestInfo.oldPassword) {
                ...
                let requestData = {newPassword: this.requestInfo.confirmPassword};
                if (this.requestInfo.oldPassword) {
                    requestData["oldPassword"] = this.requestInfo.oldPassword;
                }
                new UpdatePayPwd(requestData).send()
                    .then(res => {
                        uni.hideLoading();
                        ...
                        SetLoginModel.setPayPwd(1);
                        this.DifferenceApi.showToast(requestData["oldPassword"]
                            ? "支付密码更新成功" : "支付密码设置成功");
                        setTimeout(() => {
                            uni.navigateBack({
                                delta: 1
