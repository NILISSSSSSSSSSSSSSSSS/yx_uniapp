...
var _simpleLodash = __webpack_require__(2);
...
var CommonHandler = function () {
  function CommonHandler(binding) {
...
var matrixToTransformObj = function matrixToTransformObj(matrix) {
  ...
  values = values && values.split(')')[0];
  values = values && values.split(',');

  if (!values || !values.length) return obj;

  var _values = values,
      _values2 = _slicedToArray(_values, 6),
      a = _values2[0],
      b = _values2[1],
      c = _values2[2],
      d = _values2[3],
...
var MultiTouchGesture = function () {
  function MultiTouchGesture(el, config) {
    var _this = this;

    _classCallCheck(this, MultiTouchGesture);
    ...
    this.handleGestureStart = function (e) {
      e.preventDefault();
      var x0 = (0, _utils.pxTo750)(e.touches[0].pageX);
      var y0 = (0, _utils.pxTo750)(e.touches[0].pageY);
      var x1 = (0, _utils.pxTo750)(e.touches[1].pageX);
      var y1 = (0, _utils.pxTo750)(e.touches[1].pageY);
      _this.lastDistance = _this.distanceBetweenTwoPoints(x0, x1, y0, y1);
      _this.lastScale = 1;
      _this.lastRotation = _this.getAngle(x0, y0, x1, y1);

      _this.events.pinchstart.forEach(function (handler) {
        handler({
          scale: _this.lastScale
        });
      });

      _this.events.rotationstart.forEach(function (handler) {
        handler({
          rotation: 0
        });
      });
    ...
    this.distanceBetweenTwoPoints = function (x0, x1, y0, y1) {
      return Math.hypot(x1 - x0, y1 - y0);
    };

    this.onTouchMove = function (e) {
      var _config = _this.config,
          pointers = _config.pointers,
          threshold = _config.threshold;

      if (_this.getPointerNum(e) !== pointers) {
        return;
      }

      if (_this.lastScale < 0 || _this.lastRotation < 0) {
        _this.handleGestureStart(e);
        return;
      }
      var x0 = (0, _utils.pxTo750)(e.touches[0].pageX);
      var y0 = (0, _utils.pxTo750)(e.touches[0].pageY);
      ...
      var curDistance = _this.distanceBetweenTwoPoints(x0, x1, y0, y1);
      // let deltaDistance = curDistance - this.lastDistance;

      _this.lastScale = curDistance / _this.lastDistance;

      var curRotation = _this.getAngle(x0, y0, x1, y1);
      _this.rotation = curRotation - _this.lastRotation;

      _this.events.pinch.forEach(function (handler) {
        handler({
          scale: _this.lastScale
        });
      });
      _this.events.rotation.forEach(function (handler) {
        handler({
    ...
    this.onTouchEnd = function (e) {
      _this.events.pinchend.forEach(function (handler) {
        handler({
          scale: _this.lastScale
        });
      });
      _this.events.rotationend.forEach(function (handler) {
        handler({
          scale: _this.rotation
        });
      });
    };

    this.onTouchCancel = function (e) {
      _this.events.pinchcancel.forEach(function (handler) {
        handler({
          scale: _this.lastScale
        });
      });
      _this.events.rotationcancel.forEach(function (handler) {
        handler({
          scale: _this.rotation
        });
      });
  ...
  _createClass(MultiTouchGesture, [{
    key: 'getAngle',
    value: function getAngle(originX, originY, projectionX, projectionY) {
      var rad = Math.atan2(projectionY - originY, projectionX - originX);
...
exports.default = MultiTouchGesture;
...
var _expression = __webpack_require__(10);
...
var _fn = __webpack_require__(21);
  ...
  function Binding(options, callback) {
    ...
    switch (eventType) {
      case 'pan':
        ...
        break;
      case 'pinch':
        this._eventHandler = new _handlers.PinchHandler(this);
        break;
      case 'rotation':
        this._eventHandler = new _handlers.RotationHandler(this);
        break;
      case 'orientation':
...
exports.RotationHandler = exports.PinchHandler = exports.ScrollHandler = exports.TimingHandler = exports.OrientationHandler = exports.PanHandler = undefined;
...
var _pinch = __webpack_require__(19);
...
exports.PinchHandler = _pinch2.default;
exports.RotationHandler = _rotation2.default;
...
var _multitouch = __webpack_require__(8);
...
var PinchHandler = function (_CommonHandler) {
  _inherits(PinchHandler, _CommonHandler);

  function PinchHandler(binding) {
    _classCallCheck(this, PinchHandler);

    var _this = _possibleConstructorReturn(this, (PinchHandler.__proto__ || Object.getPrototypeOf(PinchHandler)).call(this, binding));

    _this._onPinch = function (e) {
      var scale = e.scale;
      var _this$binding$options = _this.binding.options.props,
          props = _this$binding$options === undefined ? [] : _this$binding$options;
      ...
      props.forEach(function (prop) {
        ...
        var transformed = JSON.parse(expression.transformed);
        var val = _this.binding.getValue({ s: scale }, transformed);
        _this.binding.setProperty(element, property, val);
        // stop transition
        element.style.transition = '';
        element.style.webkitTransition = '';
    ...
    _this._onPinchStart = function (e) {
      _this.binding.callback({ scale: parseInt(e.scale), state: 'start' });
    };

    _this._onPinchEnd = function (e) {
      _this.binding.callback({ scale: parseInt(e.scale), state: 'end' });
    };

    var anchor = binding.options.anchor;

    var pinchGesture = _this.pinchGesture = new _multitouch2.default(anchor, binding.options.options);
    pinchGesture.on('pinch', _this._onPinch);
    pinchGesture.on('pinchstart', _this._onPinchStart);
    pinchGesture.on('pinchend', _this._onPinchEnd);
    return _this;
  }

  _createClass(PinchHandler, [{
    key: 'destroy',
    value: function destroy() {
      var pinchGesture = this.pinchGesture;
...
var RotationHandler = function (_CommonHandler) {
  _inherits(RotationHandler, _CommonHandler);

  function RotationHandler(binding) {
    _classCallCheck(this, RotationHandler);

    var _this = _possibleConstructorReturn(this, (RotationHandler.__proto__ || Object.getPrototypeOf(RotationHandler)).call(this, binding));

    _this._onRotation = function (e) {
      ...
      props.forEach(function (prop) {
        ...
        var transformed = JSON.parse(expression.transformed);
        var val = _this.binding.getValue({ r: rotation }, transformed);
        _this.binding.setProperty(element, property, val);
        // stop transition
        element.style.transition = '';
        element.style.webkitTransition = '';
    ...
    _this._onRotationStart = function (e) {
      _this.binding.callback({ rotation: parseInt(e.rotation), state: 'start' });
    };

    _this._onRotationEnd = function (e) {
      _this.binding.callback({ rotation: parseInt(e.rotation), state: 'end' });
    };

    var anchor = binding.options.anchor;

    var rotationGesture = _this.rotationGesture = new _multitouch2.default(anchor, binding.options.options);
    rotationGesture.on('rotation', _this._onRotation);
    rotationGesture.on('rotationstart', _this._onRotationStart);
    rotationGesture.on('rotationend', _this._onRotationEnd);
    return _this;
  }

  _createClass(RotationHandler, [{
    key: 'destroy',
    value: function destroy() {
      var rotationGesture = this.rotationGesture;
...
exports.default = RotationHandler;
;
